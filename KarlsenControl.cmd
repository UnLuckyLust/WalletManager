...
@REM ----------------------
    ...
    set shortcut_icon=%cd%\KarlsenLogo.ico
@REM ---------------------------------
...
@REM Karlsen Check
set karlsendVersion=Karlsend v1.1.0
set REkarlsen=start
    ...
    if exist %Karlsen_Folder%/karlsenwallet.exe (
        if exist %Karlsen_Folder%/karlsenminer.exe (
            if exist %Karlsen_Folder%/karlsenctl.exe (
                ...
                set /p REkarlsen="[7;96m::: INPUT :::[0m Would you like to download [93m%karlsendVersion%[0m? ([96mY[0m/[96mN[0m) > "
            )
        ) else (
...
echo [7;94m::: INFO :::[0m[94m If you don't have Karlsen Daemon Running, you need to run the command - D - to connect the wallet to the node [0m
echo.
